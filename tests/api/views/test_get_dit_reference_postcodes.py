...
def test_get_dit_reference_postcodes_when_no_data(app_with_hawk_user, app_with_mock_cache):
...
def test_get_dit_reference_postcodes_next_url(
    app_with_hawk_user, app_with_mock_cache, add_dit_reference_postcodes
):
    ...
    assert (
        response.json['next']
        == 'http://localhost/api/v1/get-dit-reference-postcodes/?orientation=tabular&next-id=2'
    )
...
def test_get_dit_reference_postcodes_when_next_id_specified(
    ...
    response = make_hawk_auth_request(client, '/api/v1/get-dit-reference-postcodes/?next-id=2')
    assert response.status_code == 200
    assert response.json['next'] is None
    assert len(response.json['values']) == 1
    assert response.json['values'][0][:2] == expected_values
