...
.PHONY: run_server
run_server:
	exec gunicorn 'common.application:get_or_create()' -b 0.0.0.0:${PORT} --config 'app/config/gunicorn.conf'


.PHONY: run_dev_server
run_dev_server:
	FLASK_DEBUG=1 FLASK_APP='common.application:get_or_create()' flask run --host 0.0.0.0 --port ${PORT}
